        ...
        <div class="col-12">
            <div class="edit-branch-container">
                ...
                <div class="form-section">
                        ...
                        <div class="stat-card">
                            ...
                            <div class="stat-content">
                                <div class="stat-number">
                                    {{ number_format(($manager->contracts_sum_order_total ?? $manager->contracts()->sum('order_total')), 0, ',', ' ') }} ₸
                                </div>
                    ...
                    <div class="personnel-section">
                        <div class="personnel-item role-item">
                            <div class="personnel-icon"><i class="fas fa-user-tag"></i></div>
                        ...
                        <div class="personnel-item status-item">
                            <div class="personnel-icon"><i class="fas fa-circle"></i></div>
                            <div class="personnel-content">
                                <div class="personnel-title">Статус</div>
                                <div class="personnel-value">
                                    @if(!isset($manager->is_active) || $manager->is_active)
                ...
                <div class="form-actions">
                    ...
                    <a href="{{ route('rop.managers.edit', $manager) }}" class="btn btn-save">
...
.header-icon{width:48px;height:48px;background:#e0f2fe;color:#1ba4e9;border-radius:12px;display:flex;align-items:center;justify-content:center;font-size:20px}
...
.section-header{display:flex;align-items:center;gap:12px;margin-bottom:24px;padding-bottom:16px;border-bottom:2px solid var(--border-color);font-weight:600;font-size:16px;color:var(--text-primary)}
...
.stat-icon{width:48px;height:48px;background:#e0f2fe;border-radius:12px;display:flex;align-items:center;justify-content:center;color:#1ba4e9;font-size:20px}
.stat-content{flex:1}
.stat-number{font-size:15px;font-weight:600;color:var(--text-secondary);margin-bottom:4px}
...
.role-item .personnel-icon{background:#f3e8ff;color:#0284c7}
.branch-item .personnel-icon{background:#eff6ff;color:#2563eb}
.status-item .personnel-icon{background:#ecfdf5;color:#059669}
.personnel-content{flex:1;min-width:0}
.personnel-title{font-weight:600;font-size:13px;color:var(--text-primary);margin-bottom:8px;text-transform:uppercase;letter-spacing:.5px}
...
.rop-tag{background:#eef2ff;color:#0284c7;border:1px solid #c7d2fe}
.rop-tag:hover{background:#e0e7ff;transform:scale(1.02)}
.manager-tag{background:var(--bg-secondary);color:#475569;border:1px solid #cbd5e1}
.manager-tag:hover{background:var(--border-color);transform:scale(1.02)}
.branch-tag{background:#eff6ff;color:#2563eb;border:1px solid #bfdbfe}
.branch-tag:hover{background:#dbeafe;transform:scale(1.02)}
.code-tag{background:#f0f9ff;color:#0369a1;border:1px solid #bae6fd}
.code-tag:hover{background:#e0f2fe;transform:scale(1.02)}
.personnel-value{font-size:18px;font-weight:700}
.empty-state{color:var(--text-muted);font-size:12px;font-style:italic}
...
.personnel-item:nth-child(2){animation:fadeIn .3s ease-out .2s both}
.personnel-item:nth-child(3){animation:fadeIn .3s ease-out .3s both}
